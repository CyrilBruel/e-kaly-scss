...
router.get('/commandes/:pageNumber/:nPerPage', async function(req, res){
    const token = tools.extractToken(req.headers.authorization);
    const tokenUtilisateur = await authService.findTokenUser(token);
    var params = {crt: {}, search: "", pageNumber: req.params.pageNumber, nPerPage: req.params.nPerPage};
    ekalyService
...
router.get('/:id_commande', async function(req, res){
    const token = tools.extractToken(req.headers.authorization);
    const tokenUtilisateur = await authService.findTokenUser(token);
    var params = {crt: { _id: new ObjectId(req.params.id_commande) }, search: "", pageNumber: 1, nPerPage: 1};
    ekalyService
    .getCommandes(tokenUtilisateur.utilisateur, params)
    .then((result) => {
        res.json(responseBuilder.success(result));
